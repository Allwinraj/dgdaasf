...
async def test_verdict_contract_schema() -> None:
    out = _by_port(
        await _run(
            {"mode": "approval", "authority": "autonomous"},
...
async def test_autonomous_threshold_holds_uncertain_approve() -> None:
    ...
    out = _by_port(
        await _run(
            {"authority": "autonomous", "confidence_threshold": 0.85},
            _rows({"id": "a"}),
            llm=llm,
        )
    )
...
async def test_advisory_routes_all_to_flagged() -> None:
    ...
    out = _by_port(
        await _run(
            {"authority": "advisory", "confidence_threshold": 0.5},
            _rows({"id": "a"}),
            llm=llm,
        )
    )
...
async def test_three_verdict_ports() -> None:
    ...
    out = _by_port(
        await _run(
            {"mode": "approval", "authority": "autonomous", "confidence_threshold": 0.85},
            _rows(
...
async def test_mocked_llm_is_deterministic() -> None:
    ...
    first = _by_port(await _run({"authority": "autonomous"}, _rows(record), llm=llm))
    second = _by_port(await _run({"authority": "autonomous"}, _rows(record), llm=PromptJudgeLLM()))
...
async def test_anomaly_mode_severity_mapping() -> None:
    ...
    out = _by_port(
        await _run(
            {"mode": "anomaly", "authority": "autonomous"},
