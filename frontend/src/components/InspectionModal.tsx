...
import type { CoreAgent } from '../data/agents'
    ...
    >
      <div className="max-h-[90vh] w-full max-w-5xl overflow-hidden rounded-xl border border-white/20 bg-surface shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
        ...
        <div className="max-h-[60vh] overflow-y-auto p-gutter">
          ...
          <div className="mb-lg grid grid-cols-1 gap-sm md:grid-cols-3">
            {agent.modeDetails.map((mode) => (
              <div
                key={mode.title}
                className={`group rounded-lg border p-sm transition-colors ${
                  mode.highlight
                    ? 'box-glow-gold relative border-primary-fixed-dim/30'
                  ...
                  <Icon
                    ...
                    className={`text-[20px] ${
                      mode.highlight
                        ...
                        : 'text-on-surface-variant group-hover:text-primary-fixed-dim'
                    }`}
                  />
          ...
          <div className="relative flex min-h-[280px] flex-col items-center justify-center overflow-hidden rounded-lg border border-white/5 bg-surface-container-low p-md">
            <div
            ...
            <div className="relative z-10 flex w-full max-w-3xl items-center justify-between gap-6">
              <div className="flex flex-col gap-4">
                <InputChip icon="picture_as_pdf" label="Invoice.pdf" />
                <InputChip icon="table_view" label="PO.xlsx" />
                <InputChip icon="dataset" label="Receipts.csv" />
              </div>

              <div className="flex items-center justify-center">
                <div className="relative flex h-20 w-20 items-center justify-center rounded-full border-2 border-primary-fixed-dim/50 bg-surface-container-highest shadow-[0_0_30px_rgba(255,215,0,0.15)]">
                  <div className="flex h-16 w-16 items-center justify-center rounded-full border border-primary-fixed-dim/20 bg-surface-container-lowest">
                    <Icon name="memory" className="text-glow-gold animate-pulse text-[28px] text-primary-fixed-dim" />
                  </div>
                  <div className="absolute inset-0 animate-[spin_10s_linear_infinite] rounded-full border border-dashed border-white/20" />
                </div>
              </div>
              ...
              <div className="min-w-[200px] rounded border border-white/10 bg-surface-container p-4">
                <div className="mb-3 flex items-center gap-2 border-b border-white/5 pb-2">
                  <Icon name="mail" fill className="text-[16px] text-primary-fixed-dim" />
                  <span className="font-label-md font-bold tracking-wide text-on-surface">
                    A2A ENVELOPE
                  </span>
                </div>
                {['SLOT_01', 'SLOT_02', 'SLOT_03'].map((slot) => (
                  <div key={slot} className="flex items-center justify-between py-1">
                    ...
                    <span className="h-2 w-2 rounded-full bg-primary-fixed-dim shadow-[0_0_5px_#e9c400]" />
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="flex justify-end gap-4 border-t border-white/10 bg-surface-container/50 p-gutter">
          <button
            onClick={onClose}
            className="rounded border border-white/20 px-6 py-2.5 font-label-md text-label-md text-on-surface transition-colors hover:bg-white/5"
          >
            Close
          </button>
          <button
            onClick={() => navigate('/architect/create')}
            className="flex items-center gap-2 rounded bg-primary-container px-6 py-2.5 font-label-md text-label-md font-bold text-on-primary-container transition-colors hover:bg-primary-container/90"
          >
...
function InputChip({ icon, label }: { icon: string; label: string }) {
  return (
    <div className="flex items-center gap-2 rounded border border-white/10 bg-surface p-2 pr-4 text-sm">
      <Icon name={icon} className="text-[16px] text-on-surface-variant" />
      <span className="font-mono-label text-on-surface">{label}</span>
    </div>
  )
}
